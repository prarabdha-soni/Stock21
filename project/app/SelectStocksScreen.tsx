// screens/SelectStocksScreen.tsx
...
export default function SelectStocksScreen() {
  ...
  const { joinContest, joinedContests } = useContestStore();
  ...
  const handleSubmit = () => {
    if (selectedStocks.length === 11) {
      // Find the contest in either live or upcoming contests
      const contest = [...liveContests, ...upcomingContests].find(c => c.id === contestId);
      if (contest) {
        joinContest(contest);
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scrollView}>
        ...
        {renderCategory('Close Negative', closeNegativeStocks, '#fef3c7')}
      </ScrollView>
...
// Temporary solution - move these to a shared constants file later
const liveContests = [
  {
    id: '1',
    name: 'Mega Contest',
...
const upcomingContests = [
  ...
  },
  {
    ...
    prizePool: Math.floor(Math.random() * 500000) + 100000,
    totalSpots: Math.floor(Math.random() * 10000) + 5000,
    spotsLeft: Math.floor(Math.random() * 5000) + 1000,
    startTime: 'Tomorrow, 10 AM',
    guaranteed: Math.random() > 0.5,
    maxTeams: Math.floor(Math.random() * 10) + 5,
    firstPrize: Math.floor(Math.random() * 50000) + 10000,
    winnerPercentage: Math.floor(Math.random() * 50) + 10,
  },
];